...
from src.config.dependencies import get_current_user_id, get_accounts_email_notificator
...
def _dummy_email_sender():
    class DummySender:
        async def send_payment_email(self, *a, **k):
            pass

        async def send_cancellation_email(self, *a, **k):
            pass

        async def send_refund_email(self, *a, **k):
            pass
...
async def test_create_payment_success(
    ...
    payload = {
        ...
        "payment_items": [
            {"order_item_id": item.id, "price_at_payment": str(item.price_at_order)}
        ],
    }
...
async def test_create_payment_order_not_found(
    app, client: AsyncClient, test_user: User
):
    async def _current_user():
        return test_user.id
...
async def test_get_payment_history_only_current_user(
    ...
    o1, i1 = await _create_order_with_item(
        db_session, test_user, test_movie, status=OrderStatusEnum.PAID
    )
    p1 = Payment(
        user_id=test_user.id,
    ...
    db_session.add(
        PaymentItem(
            payment_id=p1.id, order_item_id=i1.id, price_at_payment=i1.price_at_order
        )
    )

    o2, i2 = await _create_order_with_item(
        db_session, other, test_movie, status=OrderStatusEnum.PAID
    )
    p2 = Payment(
        user_id=other.id,
        order_id=o2.id,
    ...
    db_session.add(
        PaymentItem(
            payment_id=p2.id, order_item_id=i2.id, price_at_payment=i2.price_at_order
        )
    )
    await db_session.commit()
...
async def test_get_admin_payment_history_filters(
    ...
    p2 = Payment(
        ...
        amount=o2.total_amount,
        external_payment_id="a2",
        payment_method="card",
        created_at=datetime(2024, 1, 10, tzinfo=timezone.utc),
    )
    o3, i3 = await _create_order_with_item(
        db_session, test_user, test_movie, status=OrderStatusEnum.PAID
    )
    p3 = Payment(
        user_id=test_user.id,
    ...
    db_session.add_all(
        [
            PaymentItem(
                payment_id=p1.id,
                order_item_id=i1.id,
                price_at_payment=i1.price_at_order,
            ),
            PaymentItem(
                payment_id=p2.id,
                order_item_id=i2.id,
                price_at_payment=i2.price_at_order,
            ),
            PaymentItem(
                payment_id=p3.id,
                order_item_id=i3.id,
                price_at_payment=i3.price_at_order,
            ),
        ]
    )
    await db_session.commit()
